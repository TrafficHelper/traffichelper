class Parser:
    ...
    def clean(result:str):
        ...
        had_percent = result[-1] == '%'
        result = result[:-1] if had_percent else result # Remove % for convenience term
        temporary_float = float(result)/100 if had_percent else float(result) # Must be either float or integer
