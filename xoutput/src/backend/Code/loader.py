...
def load_accidents(network):
    ...
    edge_accidents = collections.defaultdict(list)
    for i in range(len(edge_list)):
        edge_accidents[edge_list[i]] += [acc_list[i]]
    # Should now be mostly ones with occasional extras
...
def load_gadgets(network):
    ...
    # Speed Enforcers
    amount = collections.defaultdict(lambda: copy.deepcopy(
        constants.DEFAULT_GADGETS_IMPLEMENTATION[Gadget.SPEED_ENFORCER])) # Number of speed enforcement cameras to segment
...
# print(setup()) # This is commented out to prevent loading/execution when the loaded network is cached. It should be called only upon loading for the first time.
def load():
    """
